...
export default function JourneysTab({ journeys, onSave }) {
  ...
  const handleDuplicate = (journey) => {
    const dup = Object.assign({}, journey, { id: Date.now().toString() })
    onSave(journeys.concat(dup))
  ...
  const handleSaveJourney = (data) => {
    if (editingJourney) {
      ...
      onSave(journeys.concat(Object.assign({}, data, { id: Date.now().toString() })))
        ...
        {journeys.map(journey => (
          ...
          >
            ...
            <Box sx={{
              ...
              <IconButton size="small" onClick={() => handleOpen(journey)} title="Edit" aria-label="Edit journey">
                <EditIcon fontSize="small" />
              </IconButton>
              <IconButton size="small" onClick={() => handleDuplicate(journey)} title="Duplicate" aria-label="Duplicate journey">
                <ContentCopyIcon fontSize="small" />
              </IconButton>
              <IconButton size="small" color="error" onClick={() => handleDelete(journey.id)} title="Delete" aria-label="Delete journey">
