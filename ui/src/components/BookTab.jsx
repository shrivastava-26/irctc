...
export default function BookTab({ accounts, selectedAccountId, journeys, onStart, activeJobsCount, onOpenDialog }) {
  ...
  return (
    <Box sx={{ width: '100%', maxWidth: 800, mx: 'auto', minWidth: 0 }}>
      <Paper variant="outlined" sx={{ p: { xs: 1.25, sm: 1.5 }, mb: { xs: 2, sm: 3 }, display: 'flex', alignItems: { xs: 'flex-start', sm: 'center' }, flexDirection: { xs: 'column', sm: 'row' }, gap: 0.75, bgcolor: '#f0f4f8', overflowWrap: 'anywhere' }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: { xs: 'flex-start', sm: 'center' }, gap: 1, mb: 1.5, flexWrap: 'wrap' }}>
        ...
        {activeJobsCount > 0 && (
          <Button variant="text" size="small" onClick={onOpenDialog} sx={{ width: { xs: '100%', sm: 'auto' } }}>
      ...
      <Stack spacing={1} sx={{ mb: 3 }}>
        {journeys.map(journey => (
          <Paper key={journey.id} variant="outlined" sx={{ p: { xs: 1.25, sm: 1.5 }, display: 'flex', justifyContent: 'space-between', alignItems: { xs: 'flex-start', sm: 'center' }, flexDirection: { xs: 'column', sm: 'row' }, gap: 1, minWidth: 0 }}>
            <Box sx={{ minWidth: 0 }}>
              <Typography variant="body2" sx={{ fontWeight: 'bold', fontSize: '0.85rem', overflowWrap: 'anywhere', lineHeight: 1.5 }}>
                {trainSummary(journey) + ' · ' + journey.source + ' → ' + journey.destination + ' · ' + journey.travelDate + ' · ' + journey.coach + ' · ' + journey.quota}
              </Typography>
              <Typography variant="caption" color="text.secondary">
                {(journey.passengers && journey.passengers.length) || 0} Passenger(s)
                {journey.useMasterPassenger ? ' · Master Passenger' : ''}
                {' · ' + (journey.paymentPreference?.method || 'UPI') + ' configured'}
              </Typography>
            </Box>
            <Typography variant="caption" sx={{ fontWeight: 'bold', color: 'success.main', px: 1, py: 0.5, bgcolor: '#e8f5e9', borderRadius: 1, flexShrink: 0 }}>READY</Typography>
          </Paper>
        ))}
        {journeys.length === 0 && (
      ...
      <Box sx={{ display: 'flex', justifyContent: 'center', mt: { xs: 2, sm: 4 }, px: 1 }}>
        <Button fullWidth variant="contained" color="secondary" size="large" startIcon={<PlayArrowIcon />} onClick={onStart} disabled={!selectedAcc || journeys.length === 0} sx={{ maxWidth: 420, px: 4, py: 1.25, minHeight: 48 }}>
          START AUTOMATION
        </Button>
      </Box>
    </Box>
