...
class LegacyIRCTCAdapter extends IRCTCAdapter {
  ...
  async selectPassengers() {
    ...
    for (let i = 0; i < this.request.passengers.length; i += 1) {
      ...
      let nameField = null
      for (const c of name) if (await c.isVisible().catch(() => false)) { nameField = c; break }
      let ageField = null
      for (const c of age) if (await c.isVisible().catch(() => false)) { ageField = c; break }

      if (!nameField || !ageField) throw new Error('Passenger fields missing for passenger ' + (i + 1))
