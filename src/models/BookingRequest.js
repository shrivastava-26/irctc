...
const VALID_EXECUTION_TARGETS = ['API', 'LOCAL']
...
function validate(data) {
  ...
  if (data.executionTarget && !VALID_EXECUTION_TARGETS.includes(String(data.executionTarget).toUpperCase())) {
    errors.push('executionTarget must be one of: ' + VALID_EXECUTION_TARGETS.join(', '))
  }
  ...
  if (data.executionMode && !VALID_EXECUTION_MODES.includes(data.executionMode)) {
    errors.push('executionMode must be one of: NOW, SCHEDULED')
  }
  ...
  if (data.availabilityRequirement && !VALID_AVAILABILITY.includes(data.availabilityRequirement)) {
  ...
  if (Array.isArray(data.passengers)) {
    data.passengers.forEach((passenger, index) => {
      ...
      if (passenger.gender && !VALID_GENDERS.includes(passenger.gender)) {
        errors.push('passengers[' + index + '].gender must be one of: ' + VALID_GENDERS.join(', '))
      }
    })
  }
...
function normalize(data) {
  ...
  return {
    ...
    preferredTrains: Array.isArray(data.preferredTrains) ? data.preferredTrains.map(String) : [],
    backupTrains: Array.isArray(data.backupTrains) ? data.backupTrains.map(String) : [],
    trainSelectionPolicy: selectionPolicy,
    coach: String(data.coach).toUpperCase(),
    boardingStation: data.boardingStation ? String(data.boardingStation).toUpperCase() : null,
...
module.exports = { validate, normalize, VALID_EXECUTION_TARGETS }
